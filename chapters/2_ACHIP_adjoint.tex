...
\figdef{DLA_definition.pdf}{DLA_def}{Diagram outlining the system setup for side-coupled DLA with an arbitrary dielectric structure $\eps(x,y)$ (green).  A charged particle moves through the vacuum gap with speed $\beta c_0$.  The periodicity is set at $\beta \lambda$ where $\lambda$ is the central wavelength of the laser pulse.}
...
\eqdef{Gintro}{G = \frac{1}{T}\int_0^{T}{ E_{||}(\vr(t),t)\ dt},}
%
...
\eqdef{}{\bfx(\vr,t) = \real{\bfx(\vr)e^{i\omega t}},}
...
\figdef{eta_definition.png}{eta_def}{Defining Eta}
